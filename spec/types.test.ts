...
describe("Types", () => {
  ...
  it("should be able to define types with explicit default values", () => {
    ...
    expect((env.num as JacquesNumber).__constant__).toBe(true);
    expect((env.str as JacquesString).value).toBe("Hello");
    expect((env.str as JacquesString).__constant__).toBe(true);
    expect((env.map as JacquesRecord).properties).toEqual({
      key: new JacquesString("value"),
    });
    expect((env.map as JacquesRecord).__constant__).toBe(true);
    expect((env.arr as JacquesArray).elements).toEqual([
      new JacquesNumber(1),
      new JacquesNumber(2),
      new JacquesNumber(3),
    ...
    expect((env.bool as JacquesBoolean).__constant__).toBe(true);
  });
